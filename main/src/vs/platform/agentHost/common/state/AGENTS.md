...
This directory contains the HappyDev-facing wrappers around the Agent Host
...
- `sessionState.ts`, `sessionActions.ts`, `sessionReducers.ts`, and
  `sessionProtocol.ts` are HappyDev-facing wrappers and re-exports.
- `protocol/**` is generated from the sibling `agent-host-protocol` repo by
  ...
  banner; update the source protocol repo and sync the copy into HappyDev.
...
- Session teardown uses `changeset/cleared` plus the corresponding
  ...
  action in the HappyDev protocol copy.

## Updating generated protocol types

1. Update the source files in the sibling `agent-host-protocol` repo.
2. Run `npx tsx scripts/sync-agent-host-protocol.ts` from the HappyDev repo.
3. If HappyDev consumers need short aliases or type guards, update the wrapper
...
4. Re-export the new action from `sessionActions.ts` when HappyDev callers need
   the type directly.
