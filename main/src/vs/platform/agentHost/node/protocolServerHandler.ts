...
export interface IProtocolServerConfig {
	...
	/**
	 * Whether to expose HappyDev extension methods outside the Agent Host Protocol.
...
export class ProtocolServerHandler extends Disposable {
	...
	private _handleNewConnection(transport: IProtocolTransport): void {
		...
		disposables.add(transport.onMessage(msg => {
			if (isJsonRpcRequest(msg)) {
				...
				// The HappyDev upgrade request rides on the same transport but
	...
	private _handleInitialize(
		...
		if (!negotiated) {
			const data: UnsupportedProtocolVersionErrorDataEx = {
				...
				// host was spawned by a HappyDev CLI that is listening for
	...
	/**
	 * Forwards a client's upgrade request to the hosting HappyDev CLI's
	...
	private _handleRequest(client: IConnectedClient, method: string, params: unknown, id: number): void {
		...
		// HappyDev extension methods (not in the typed protocol maps yet)
	...
	/**
	 * Handle HappyDev extension methods that are not yet part of the typed
